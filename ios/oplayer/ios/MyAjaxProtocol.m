...
- (void)startLoading
{
    ...
    self.task = [theManager dataTaskWithRequest:newRequest completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        //  DEBUG: TODO：是否两次响应检测
        if (taskResponsed){
            DLog(@"NetworkError double responsed...");
        }
        taskResponsed = YES;
        ...
        {
            DLog(@"response %@ responseObject %@ error %@",NSStringFromClass([response classForCoder]), NSStringFromClass([responseObject classForCoder]), error);
        }
        //  REMARK：这个error和NSURLSession代理中出现的error不同，这个仅仅是AF decode 原始数据产生的错误。
        if(error)
        {
            DLog(@"NetworkError response %@ responseObject %@ error %@",NSStringFromClass([response classForCoder]), NSStringFromClass([responseObject classForCoder]), error);
